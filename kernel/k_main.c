...
void _kernel_init(multiboot_info_t* mb_info)
{
     ...
     *   (v) 分配新的栈空间
     *       调整映射：
     *   ( )    + 映射memory map（APCI, APIC, IO映射）（以后）
     *   (v)    + 释放hhk_init所占据的空间
    */
    ...
    for (unsigned int i = 0; i < map_size; ++i) {
        ...
        if (mmap.type == MULTIBOOT_MEMORY_AVAILABLE) {
            ...
            printf("[MM] Free %u pages start from 0x%x\n",
                   map[i].len_low >> 12,
                   pg & ~0x0fffU);
        }
    }
    ...
    */
    ...
    for (size_t i = 0; i < vga_buf_pgs; ++i) {
        vmm_map_page(VGA_BUFFER_VADDR + (i << 12), VGA_BUFFER_PADDR + (i << 12), PG_PERM_RW, PG_PERM_RW);
    }
    /* 更新VGA缓冲区位置至虚拟地址 */
    ...
    for (size_t i = 0; i < (K_STACK_SIZE >> 12); ++i) {
        vmm_alloc_page(K_STACK_START + (i << 12), PG_PERM_RW, PG_PERM_RW);
    }
